...
list_stock = [PETR4,VALE3,ITUB4,ABEV3,BOVA11]
...
for j in range(1):
    ...
    for stock_index in list_stock:
        ...
        for year in list_year:
            ...
            df = ta.add_all_ta_features(df, open="Open_", high="High_", low="Low_", close="Close_", volume="Volume", fillna=True)
    ...
    result = pd.DataFrame(columns=['ativo','modelo','bandhold','ma'])
    i = 0
    for stock_name in df_dict.keys():
        ...
        df_['Signal'] = buy_sell_position_(np.array(df_['Label']))
        entries = df_['Signal']==1
        exits = df_['Signal']==-1
        pf = vbt.Portfolio.from_signals(df_['Close'], entries, exits)
        print('Modelo:')
        ...
        df_['Signal'] = buy_sell_position(df_.index,np.array(df_['Label_pred_']),df_['Close'],0.1)
        entries = df_['Signal']==1
        exits = df_['Signal']==-1
        pf = vbt.Portfolio.from_signals(df_['Close'], entries, exits)
        print('Modelo:')
        ...
        df_['Signal_'] = buy_sell_position_(np.array(df_['Label_pred_']))
